...
require_once("../lib/date-util.php");

class ReservationArchives implements JsonSerializable {
	 ...
	 * @param  PDO $pdo PDO connection object
	 * @param  string $teamName
	 * @return SplFixedArray of reservations by Team Name
	 * @throws PDOException when mySQL related errors occur
	...
	public static function getReservationsByTeamName(PDO $pdo, string $teamName) {
		...
		try {
			...
			while($row = $statement->fetch() !== false) {
				$reservation = new Users($row["resId"], $row["teamName"], $row["resDateTime"], $row["courts"], $row["resType"]);
	 ...
	 * @throws Exception when other exceptions are caught
	 */
	public static function getReservationsByDate(PDO $pdo, DateTime $resDate) {
	 ...
	 * @return ReservationArchives|null reservation found or null if not found
	 * @throws PDOException when mySQL related errors occurs
	...
	public static function getReservationById(PDO $pdo, int $resId) {
		...
		try {
			...
			if($row !== false) {
				$reservation = new ReservationArchives($row["resId"], $row["teamName"], $row["resDateTime"], $row["courts"], $row["resType"]);
